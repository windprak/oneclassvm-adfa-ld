...
df2.to_csv(path+"train3dim.csv", index=False, header=None)   


graph=np.empty([734,3])
for k in range(0,734):
         numb=format(k,'04')
         text_file = open ("data/Attack_Data/"+numb+".txt")
         trace = text_file.read()
         word_list = [int(i) for i in trace.split()]
         x=set()
         probl= list()
...
df2 = pd.DataFrame(graph)
df2.to_csv(path+"attackdata3dim.csv", index=False, header=None)   



graph=np.empty([4372,3])
...
df2 = pd.DataFrame(graph)
df2.to_csv(path+"datatest3dim.csv", index=False, header=None)   
